...
(define bernoulli*
  (let ([num-global-bs : (Refine [i : Fixnum] (= i 200)) 200]) ;; Number of globally memoized Bernoulli numbers 
    (let ([global-bs : (Refine [v : (Vectorof Exact-Rational)] (= (len v) num-global-bs))
                     (make-vector num-global-bs (ann 0 Exact-Rational))]) ;; Globally memoized numbers
      (safe-vector-set! global-bs 0 1)
      (safe-vector-set! global-bs 1 -1/2)
      (λ ([n : Natural]) : Exact-Rational
