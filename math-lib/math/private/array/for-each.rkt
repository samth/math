...
(define-syntax-rule (for-each-array+data-index ds-expr f-expr)
  (let ([ds : Indexes  ds-expr])
    (let ([dims : (Refine [i : Index] (= i (len ds)))  (vector-length ds)])
      (define-syntax-rule (f js j)
        ((ann f-expr (Indexes Nonnegative-Fixnum -> Void)) js j))
      (cond
        [(= dims 0)  (f ds 0)]
        [else
         (define: js : (Refine [v : Indexes] (= dims (len v))) (make-vector dims 0))
         (cond
           [(= 1 dims)
            (define: d0 : Index (safe-vector-ref ds 0))
           ...
           [(= 2 dims)
            (define: d0 : Index (safe-vector-ref ds 0))
            (define: d1 : Index (safe-vector-ref ds 1))
            (let: j0-loop : Void ([j0 : Nonnegative-Fixnum  0]
           ...
           [else  (let: i-loop : Nonnegative-Fixnum ([i : Nonnegative-Fixnum  0]
                                                     [j : Nonnegative-Fixnum  0])
                    (cond [(i . < . dims)
                           (define: di : Index (safe-vector-ref ds i))
                ...
                (let: j1-loop : Void ([j1 : Nonnegative-Fixnum  0])
                  (cond [(j1 . < . d1)
                        ...
                        [else
                         (j0-loop (+ j0 1))]))))]
           [else
            (let: i-loop : Void ([i : Nonnegative-Fixnum  0])
              (cond [(i . < . dims)
                     (define: di : Index (safe-vector-ref ds i))
    ...
    (let ([dims : (Refine [i : Index] (= i (len ds)))  (vector-length ds)])
      ...
      (define: size : Nonnegative-Fixnum
        (let: loop : Nonnegative-Fixnum ([k : Nonnegative-Fixnum  0] [size : Nonnegative-Fixnum  1])
          (cond [(k . < . dims)  (loop (+ k 1) (unsafe-fx* size (safe-vector-ref ds k)))]
                [else  size])))
      (cond [(= size 0)  (ann (vector) (Vectorof A))]
            [else
             (define: js0 : (Refine [v : Indexes] (= dims (len v))) (make-vector dims 0))
             (define: vs : (Refine [v : (Vectorof A)] (= size (len v))) (make-vector size (g js0 0)))
